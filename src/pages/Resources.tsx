...
const COLLEGES = [
  ...
  'College of Engineering, Pune (COEP) – Autonomous',
  ...
  'Maharashtra Institute of Technology, Aurangabad (MITA) – Autonomous',
  'Government College of Engineering, Nagpur (GCOEN) – Autonomous',
  ...
  'AISSMS Institute of Information Technology (AISSMSIIT, Pune)',
  'College of Engineering Pune (COEP Tech University)',
  'Defence Institute of Advanced Technology (DIAT), Pune',
  'MIT World Peace University (MIT-WPU)',
  'Vishwakarma Institute of Technology (VIT Pune)',
  'Bansilal Ramnath Agarwal Charitable Trust’s Vishwakarma Institute of Technology (BRACT-VIT)',
...
const ResourcesPage = () => {
  ...
  // Add state for expandable containers, filters, and pagination
  const [expandedTypes, setExpandedTypes] = useState<{ [key: string]: boolean }>({});
  const [typeFilters, setTypeFilters] = useState<{ [key: string]: { college: string, branch: string, year: string, semester: string, subject: string } }>({});
  const [typePagination, setTypePagination] = useState<{ [key: string]: number }>({});
  ...
  const fetchResources = async (pageNum = 1, append = false) => {
    try {
      setIsLoading(true);
      setError('');
      const response = await axios.get(`${API_BASE_URL}/documents/?page=${pageNum}&page_size=${PAGE_SIZE}`);
      const mappedResources = response.data.results.map((doc: any) => ({
        id: doc.id,
      ...
      if (append) {
        ...
        setFilteredResources(prev => [...prev, ...mappedResources]);
      } else {
  ...
  useEffect(() => {
    ...
    // Apply tab filter
    if (activeTab !== 'all') {
      filtered = filtered.filter(resource => resource.resource_type === activeTab);
    }
  ...
  const groupResourcesByType = (resources: Resource[]) => {
    ...
    resources.forEach(resource => {
      if (RESOURCE_TYPES.includes(resource.resource_type)) {
  ...
  const toggleExpandType = (type: string) => {
    ...
    // Initialize pagination and filters if not set
    if (typePagination[type] === undefined) {
      setTypePagination(prev => ({ ...prev, [type]: 10 }));
    }
    if (!typeFilters[type]) {
      setTypeFilters(prev => ({
        ...prev,
        [type]: { college: '', branch: '', year: '', semester: '', subject: '' }
      }));
    }
  };

  // Handle filter change
  const handleTypeFilterChange = (type: string, field: string, value: string) => {
    setTypeFilters(prev => ({
      ...prev,
      [type]: { ...prev[type], [field]: value }
    }));
    setTypePagination(prev => ({ ...prev, [type]: 10 })); // Reset pagination on filter change
  };

  // Filter resources for each type
  const getFilteredTypeResources = (type: string) => {
    let resources = groupedResources[type] || [];
    const filters = typeFilters[type] || {};
    if (filters.college) resources = resources.filter(r => r.college === filters.college);
    if (filters.branch) resources = resources.filter(r => r.branch.toLowerCase().includes(filters.branch.toLowerCase()));
    if (filters.year) resources = resources.filter(r => r.year === filters.year);
    if (filters.semester) resources = resources.filter(r => r.semester === filters.semester);
    if (filters.subject) resources = resources.filter(r => r.subject.toLowerCase().includes(filters.subject.toLowerCase()));
    return resources;
  };
    ...
    <div className="min-h-screen bg-gray-50 py-8 px-4 sm:px-6 lg:px-8">
      <div className="max-w-7xl mx-auto">
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 mb-8">
          {/* LEFT SIDE: Search, Tabs, and Latest Resources */}
          <div className="lg:col-span-2">
            ...
            <div className="bg-white rounded-xl shadow-sm border border-gray-200 p-6 mb-8">
              <div className="relative mb-6">
                ...
                <input
                  type="text"
                  placeholder="Search resources by title, subject, college, etc..."
              ...
              <div className="flex flex-wrap gap-2 mb-6">
                ...
                {RESOURCE_TYPES.map(type => (
                  <button
                    key={type}
                    className={`px-4 py-2 rounded-full text-sm font-medium ${activeTab === type ? 'bg-blue-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
            ...
            {/* Latest Resources UI - placed directly below search/tabs */}
            ...
            {/* Expandable containers for each resource type */}
            <div className="space-y-6 mb-8">
              {RESOURCE_TYPES.map(type => {
                const expanded = expandedTypes[type];
                const filters = typeFilters[type] || { college: '', branch: '', year: '', semester: '', subject: '' };
                const filteredTypeResources = getFilteredTypeResources(type);
                const shownResources = filteredTypeResources.slice(0, typePagination[type] || 10);
                return (
                  <div key={type} className="border border-gray-200 rounded-lg bg-white shadow-sm">
                    {/* Custom Header */}
                    <button
                      className={`w-full flex items-center justify-between px-6 py-4 text-lg font-bold transition-colors
                        ${expanded ? 'bg-blue-50 text-blue-700 border-b border-blue-200' : 'bg-gray-50 text-gray-900 hover:bg-blue-50'}
                      ...
                      <div className="px-6 pb-6 pt-4">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-5 gap-4 mb-4">
                          <select
                            ...
                            onChange={e => handleTypeFilterChange(type, 'college', e.target.value)}
                          ...
                          <input
                            ...
                            onChange={e => handleTypeFilterChange(type, 'branch', e.target.value)}
                            placeholder="Branch"
                            className="px-2 py-1 border rounded"
                          />
                          <select
                            value={filters.year}
                            onChange={e => handleTypeFilterChange(type, 'year', e.target.value)}
                            className="px-2 py-1 border rounded"
                          >
                          ...
                          <select
                            ...
                            onChange={e => handleTypeFilterChange(type, 'semester', e.target.value)}
                            className="px-2 py-1 border rounded"
                          >
                          ...
                          <input
                            ...
                            onChange={e => handleTypeFilterChange(type, 'subject', e.target.value)}
                            placeholder="Subject"
                            className="px-2 py-1 border rounded"
                          />
                        </div>
                        ...
                        {shownResources.length === 0 ? (
                          <div className="text-gray-500 py-4 text-center">No resources found.</div>
                        ) : (
                          <ul className="divide-y divide-gray-200">
                              ...
                              <li key={resource.id} className="py-4">
                                <div className="flex items-center">
                                  ...
                                  <div className="ml-4 flex-1 min-w-0">
                                    ...
                                    <div className="mt-1 flex items-center text-sm text-gray-500">
                                      <span className="mr-2">{FILE_TYPE_MAP[resource.file_type] || resource.file_type.toUpperCase()}</span>
                                      <span className="mx-1">•</span>
                                      <span>{formatFileSize(resource.size)}</span>
                                      <span className="mx-1">•</span>
                                      <span>{formatDate(resource.uploaded_at)}</span>
                                      {resource.subject && (<><span className="mx-1">•</span><span>{resource.subject}</span></>)}
                                      {resource.college && (<><span className="mx-1">•</span><span className="truncate max-w-xs">{resource.college}</span></>)}
                                    </div>
                                    {resource.description && (
                                      <p className="mt-1 text-sm text-gray-500 truncate">{resource.description}</p>
                                    )}
                                  </div>
                                  <div className="ml-4 flex-shrink-0 flex space-x-2">
                                    <button
                        ...
                        {/* Load More Button */}
                        {shownResources.length < filteredTypeResources.length && (
                          <div className="flex justify-center mt-4">
                            <button
                              className="px-4 py-2 bg-blue-600 text-white rounded hover:bg-blue-700"
                              onClick={() => setTypePagination(prev => ({
                                ...prev,
                                [type]: (prev[type] || 10) + 10
                              }))}
                            >
                              Load More
                            </button>
